...
namespace TaskDroid {
    ...
    void AndroidStackMachine::addAction(Activity* activity, Intent* intent, bool finish) {
        ...
        if (flag) {
            actionMap[activity].insert(pair(intent, finish));
        }
    }
    ...
    bool AndroidStackMachine::minimize() {
        ...
        unordered_set<string> afts({mainActivity -> getAffinity()});
        while (work.size()) {
            for (auto act : work) {
                if (actionMap.count(act) == 0) continue;
                for (auto& [intent, finish] : actionMap.at(act)) {
                    if (AndroidStackMachine::isNewMode(act, intent))
                        afts.insert(intent -> getActivity() -> getAffinity());
        ...
        for (auto activity : acts) {
            ...
            if (afts.count(activity -> getAffinity()) == 0) 
                activity -> setAffinity(mainActivity -> getAffinity());
