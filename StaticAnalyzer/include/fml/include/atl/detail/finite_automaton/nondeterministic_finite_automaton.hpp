...
namespace atl::detail {
    ...
    public:
        ...
        void remove_transition(State s, const Symbol& c) {
            OutTransitionIter first, last;
            tie(first, last) = out_transitions(*this, s);
            vector<Transition> transitions;
            for (; first != last; first++) {
                transitions.emplace_back(*first);
            }
            for (auto& transition : transitions) {
                auto& prop = atl::get_property(*this, transition);
                if (prop != c) remove_transition(transition);
            }
        }
...
namespace atl {
    ...
    template <NFA_PARAMS>
    void
    remove_transition(NFA& a,
                      typename NFA::State s,
                      const NFA_SYMBOL& c) {
        set_modified_flag(a);
        return a.remove_transition(s, c);
    }
