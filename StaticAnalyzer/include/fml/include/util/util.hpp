...
namespace util {
    ...
    template<class T>
    static void combine(const vector<T>& datas, size_t n, size_t m, vector<size_t>& indexes, size_t M,std:: unordered_set<std::unordered_set<T> >& coms) {
        for (auto i = n; i >= m; i--) {
            indexes[m - 1] = i - 1;
            if (m > 1) {
                combine(datas, i - 1, m - 1, indexes, M, coms);
            } else {
                std::unordered_set<T> com;
                for (int j = M - 1; j >= 0; j--) {
    ...
    template<class T>
    static void combine(const vector<T>& datas, size_t m, std::unordered_set<std::unordered_set<T> >& coms) {
        vector<size_t> indexes(m);
        auto n = datas.size();
        combine(datas, n, m, indexes, m, coms);
    }
    ...
    template<class T>
    static void power_set(const std::unordered_set<T>& datas, std::unordered_set<std::unordered_set<T> >& pows) {
        vector<T> new_datas(datas.begin(), datas.end());
        for (size_t i = 1; i <= new_datas.size(); i++) {
            combine(new_datas, i, pows);
        }
    }
